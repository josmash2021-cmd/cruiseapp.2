...
- Picker "Set your drop-off" = `RideRequestScreen(pickerMode: true)` empujado desde `pickup_dropoff_search_screen.dart`. Latch anti-snap `_userTookCamera`; gate GPS `_gpsMayMoveCamera` (incluye `_mapMounted` desde 2026-08-09 — una sheet tapada NO mueve cámara); reverse-geocode desde el CENTRO del mapa (`_pickerOnCameraIdle`). Si el MapWidget se recrea (GPU/handoff), bootea desde `_lastCam*` — **sembrado en initState desde el handoff/boot (build 573 report): null hasta el primer evento de cámara = recreación bootea al seed (snap al seed mid-drag); si iOS no dispara `onCameraChangeListener` en gestos, null para siempre** — NUNCA del handoff/GPS una vez sembrado. **`_mapSurfaceOwner` es POR INSTANCIA** (`RideRequest-N`, 2026-08-09): sheet y picker son AMBAS RideRequestScreen y el picker va encima de la sheet — con el id estático compartido el coordinator saltaba el revoke (`_owner == owner`), la sheet de abajo quedaba viva y su `_initLocation` tardío (fix GPS frío, hasta 10 s) volaba la cámara a GPS top-down mientras el rider arrastraba: el snap-back de "recién abro la app". Test guardián: `test/picker_camera_guard_test.dart` (si falla, el snap-back volvió).
