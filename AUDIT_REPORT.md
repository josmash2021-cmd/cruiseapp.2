...
|---|-----------|---------|--------|-----|
| C1 | `backend/routers/auth.py:259-327` | **Hardcoded demo accounts** (`+15550001234`, `+15550005678`, `applereview@cruiseride.com`, `appledriver@cruiseride.com`) with auto-approval and vehicle creation if `ENABLE_DEMO_ACCOUNTS` is set | Complete account takeover of demo accounts including approved driver | Remove demo accounts from production code; gate behind `DEBUG=1` only |
