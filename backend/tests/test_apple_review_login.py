...
(23456789) log straight in without an OTP row. Pinned here: rider-only,
...
async def test_apple_review_login_creates_rider_without_code(client: AsyncClient, db):
    ...
    resp = await client.post(
        "/auth/phone-login",
        json={"phone": "23456789", "code": "", "role": "rider"},
    ...
    assert data["user"]["phone"] == "+123456789"
...
async def test_apple_review_login_reuses_the_same_account(client: AsyncClient, db):
    ...
    second = await client.post(
        "/auth/phone-login",
        json={"phone": "+123456789", "code": "000000", "role": "rider"},
        headers=_make_auth_headers(),
    )
...
async def test_apple_review_digits_never_create_a_driver(client: AsyncClient, db):
    ...
    validation and die (8 digits is not a US number, no code on file)."""
    resp = await client.post(
        "/auth/phone-login",
        json={"phone": "23456789", "code": "123456", "role": "driver"},
        headers=_make_auth_headers(),
    )
    assert resp.status_code == 400

    from main import User
    r = await db.execute(select(User).where(User.phone == "+123456789"))
