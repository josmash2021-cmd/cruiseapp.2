...
class ApiService {
  ...
  /// Get wallet payout methods for cashout.
  /// Returns: [{id, method_type, display_name, is_default, created_at}]
  static Future<List<Map<String, dynamic>>> getWalletPayoutMethods() async {
    final h = await _authHeaders();
    final res = await _client
        .get(
          Uri.parse('$_baseUrl/wallet/payout-methods'),
          headers: h,
        )
        .timeout(const Duration(seconds: 15));
    final data = _parse(res);
  ...
  /// Returns: [{id, amount, type, description, created_at}]
  static Future<List<Map<String, dynamic>>> getWithdrawalHistory({
    ...
    final res = await _client
        .get(
          Uri.parse('$_baseUrl/wallet/withdrawals?limit=$limit&offset=$offset'),
          headers: h,
        )
        .timeout(const Duration(seconds: 15));
    final data = _parse(res);
