...
class OfflineService extends ChangeNotifier {
  ...
  /// Save queued updates to local storage
  Future<void> _saveQueuedUpdates() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final json = _queuedUpdates.map((u) => u.toString()).toList();
      await prefs.setStringList('offline_queue', json);
    } catch (e) {
      debugPrint('[Offline] Error saving queue: $e');
    }
  }
  
  /// Load queued updates from local storage
  Future<void> _loadQueuedUpdates() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final json = prefs.getStringList('offline_queue') ?? [];
      // In production, deserialize proper JSON here
      _queuedUpdates = json.map((u) => {'data': u}).toList();
    } catch (e) {
